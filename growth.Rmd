...
r_distribution = rnorm(mean = 0.01, sd = r*.10, n = np)
g_distribution = rnorm(mean = 2, sd = g*.10, n = np)
threshold_distribution = rnorm(mean = 50, sd = threshold*.10, n = np)
K_distribution = rnorm(mean = 250, sd = K*.10, n = np)

#create the first dataframe 
X1 = cbind.data.frame(r = r_distribution, 
                      g = g_distribution, 
                      threshold = threshold_distribution, 
                      K = K_distribution)

#change the r, g, and K values 
r = rnorm(mean = 0.02, sd = 0.01*.10, n = np)
g = rnorm(mean = 3, sd = 2*.10, n = np)
threshold = rnorm(mean = 50, sd = 50*.10, n = np)
K = rnorm(mean = 300, sd = 250*.10, n = np)
...
Pinitial = 10 #forest size (initial population in units of kgC)
...
parms = list(r=as.data.frame(sens_P$X)$r[1], g=as.data.frame(sens_P$X)$g[1], threshold=as.data.frame(sens_P$X)$threshold[1], K=as.data.frame(sens_P$X)$K[1])

result = ode(C, simtimes, forest_growth, parms)
